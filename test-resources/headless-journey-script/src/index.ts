...
describe(
  `Completes ${input.journeyIdentifier} OAuth journey`,
  { concurrency: false },
  () => {
    ...
    it("gets an auth code", async () => {
      ...
      assert(
        "authorizationCode" in parsedBody &&
          "redirectionURI" in parsedBody &&
          "state" in parsedBody,
      );
    ...
    it("retrieves the verifiable credential", async () => {
      ...
      const [headerJSON, payloadJSON] = verifiableCredential
        .split(".")
        .map(base64Decode);

      const [header, payload] = [headerJSON, payloadJSON].map((v) =>
        JSON.parse(v),
      );

      assert(header.typ && header.alg && header.kid);
      assert(
        payload.sub && payload.nbf && payload.iss && payload.vc && payload.jti,
      );
